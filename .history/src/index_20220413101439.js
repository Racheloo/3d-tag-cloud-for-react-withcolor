...
export default class TagCloud extends React.Component {
	...
	move(tagName) {
		...
		const tags = tagName.map((tag, i) => {
			...
			const colorArr = ['#7cb5ec', '#434348', '#90ed7d', '#f7a35c', '#8085e9', '#f15c80', '#e4d354', '#2b908f', '#f45b5b', '#91e8e1', '#058DC7', '#50B432', '#ED561B', '#DDDF00', '#24CBE5', '#64E572', '#FF9655', '#FFF263', '#6AF9C4', '#D47F00', '#00FFFF', '#D4FF55', '#4572A7', '#AA4643', '#89A54E', '#80699B', '#3D96AE', '#DB843D', '#92A8CD', '#A47D7C', '#7FBF55',
			...
			const colors = this.state.colors.length > 0 ? this.state.colors : colorArr;

			const num = this.state.colors.length > 0 ? Math.floor(Math.random()*this.state.colors.length) : 50;
			var tagProps = {
				x: x,
				y: y,
				z: z,
